...
export default {
  ...
  detectSerialData(ConnectionPort, ConnectionBaudrate) {
    ...
    var config_data_resolution = Config.data.resolution;
    var config_zero = 150;
    ...
    this.pyscript = new BufferedProcess({
      ...
      stdout: (data) => {
        ...
        for (r = 0; r < split_data.length; r++) {
          if (split_data[r].length <= 2) {
            split_data.splice(r, 1);
          }
        }
        ...
        for (j = 0; j < split_data.length; j++) {
          ...
          if (isJSON(split_data[j])) {
            ...
            if (stored_values.length < config_data_resolution) {
            ...
            time_scale = this.PlotterPanel.element.clientWidth / (config_data_resolution - 1);
            ...
            if (stored_values[stored_values.length - 1].hasOwnProperty('ng')) { // We have a config !
              ...
              config_data_resolution = stored_values[stored_values.length - 1].g[0].pd;
