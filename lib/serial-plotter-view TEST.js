...
export default class SerialPlotterView {
  ...
  constructor(serializedState) {
    ...
    tb_config = document.createElement("BUTTON");
    tb_config.classList.add('btn');
    tb_config.classList.add('icon-logout');
    tb_config.style.float = 'right';
    tb_config.onclick = function() {
      atom.packages.loadedPackages["serial-plotter"].mainModule.serialPlotterView.config()
    };
    tb_config_text = document.createTextNode("Config");
    tb_config.appendChild(tb_config_text);
    tb_buttonBox.append(tb_config);
    ...
    // Create Config Box
    config_area = document.createElement('div');
    config_area.style.display = 'none';
    config_area.style.height = Config.window.height + 'px';
    config_area.style.display = 'table-cell';
    config_area.style.verticalAlign = 'middle';
    config_area.style.textAlign = 'center';
    config_area.style.backgroundColor = 'rgb(44, 48, 55)';
    content_area.append(config_area);
  ...
  disconnect() {
    ...
    atom.packages.loadedPackages["serial-plotter"].mainModule.serialPlotterView.hideConfigLayer();
    atom.packages.loadedPackages["serial-plotter"].mainModule.serialPlotterView.showSettingsLayer();
  }

  config() {
    atom.packages.loadedPackages["serial-plotter"].mainModule.serialPlotterView.hideDrawingLayer();
    atom.packages.loadedPackages["serial-plotter"].mainModule.serialPlotterView.hideSettingsLayer();
    atom.packages.loadedPackages["serial-plotter"].mainModule.serialPlotterView.showConfigLayer();
  }
