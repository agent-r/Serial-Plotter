...
*/
	...
	detectSerialData() {
		...
		console.log(atom.workspace.panelContainers.bottom.element.clientWidth);
		...
		var time_scale = Config.window.width / Config.data.resolution;
		...
		this.pyscript = new BufferedProcess({
			command: 'python3',
			args: [cmd, '-p', Config.connection.port, '-b', Config.connection.baudrate],
			stdout: (data) => {

				if (killer) {
					console.log("Kill BufferedProcess ...");
					this.pyscript.kill();
				}
				...
				if (data != 0) {
					...
					if (skip_empty == false) {
						...
						config_zero = (((Config.window.height - Config.window.topline - Config.window.bottomline) / 2) + Config.window.topline) + (((scale_lowest + scale_highest) / 2) * config_scale);

						drawing.clearRect(0, 0, Config.window.width, Config.window.height);
						...
						drawing.lineTo(Config.window.width, Config.window.topline - 5);
						drawing.moveTo(0, config_zero);
						drawing.lineTo(Config.window.width, config_zero);
						drawing.moveTo(0, Config.window.height - Config.window.bottomline + 5);
						drawing.lineTo(Config.window.width, Config.window.height - Config.window.bottomline + 5);
						...
						for (m = 0; m < config_size; m++) {
							...
							drawing.fillText(config_lable[m] + ' : ' + stored_values[stored_values.length - 1].g[0].d[m], ((Config.window.width / 6.0) * (m + 1) + 15), 15);
							drawing.beginPath();
						...
						drawing.fillRect(30, Config.window.height - Config.window.bottomline - 20 + 5, 90, 20);
						...
						drawing.fillText(scale_lowest, 38, Config.window.height - Config.window.bottomline - 1);
						drawing.stroke();

					} else {
						console.log('wrong input : skip this line ...');
					}
				} else {
