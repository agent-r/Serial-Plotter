...
export default {
	...
	detectSerialData(ConnectionPort, ConnectionBaudrate) {
		...
		console.log("Conecting ... Port : " + ConnectionPort + "    Baudrate : " + ConnectionBaudrate);
				...
				if (data != 0) {
					...
					if (skip_empty == false) {
						...
						if (stored_values[stored_values.length - 1].hasOwnProperty('ng')) { // We have a config !
							...
							time_scale = this.PlotterPanel.element.clientWidth / Config.data.resolution;
						...
						drawing.clearRect(0, 0, this.PlotterPanel.element.clientWidth, Config.window.height);
						...
						drawing.lineTo(this.PlotterPanel.element.clientWidth, Config.window.height - Config.window.bottomline + 5);
