				...
				if (data != 0) {
					...
					if (skip_empty == false) {
						...
						for (m = 0; m < config_size; m++) {
							...
							drawing.fillText(config_lable[m] + ' : ' + stored_values[stored_values.length - 1].g[0].d[m], 200 * m + 200, 15);
							drawing.beginPath();
							drawing.moveTo(0, config_zero - (stored_values[0].g[0].d[m] * config_scale));
							for (i = 1; i < stored_values.length; i++) {
								drawing.lineTo(i, config_zero - (stored_values[i].g[0].d[m] * config_scale));
							}
							drawing.stroke();
						}
					} else {
