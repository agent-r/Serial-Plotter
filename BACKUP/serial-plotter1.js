  ...
  detectSerialData() {
    ...
    var d_pyscript = new BufferedProcess({
      ...
      stdout: (d_data) => {
        ...
        d_split_result = d_result.split('#');
        for (j = 0; j < d_split_result.length; j++) {
          if (d_split_result[j].length > 2) {
            while ((d_split_result[j].length > 4) && (d_split_result[j].charAt(0) != '{')) { d_split_result[j] = d_split_result[j].substr(1); }
            console.log(d_split_result[j]);
            d_parsed_result = JSON.parse(d_split_result[j]);



            message.textContent = d_parsed_result.g[0].d[0];

            if (value.length < 1200) {
              value.push(d_parsed_result.g[0].d[0]);
            } else {
              value.shift();
              value.push(d_parsed_result.g[0].d[0]);
            }
        ...
        // d_parsed_result = JSON.parse(d_split_result);
        // message.textContent = d_parsed_result.g[0].d[0];
